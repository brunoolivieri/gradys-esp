...
void loop() {
  while(Serial.available()){
    commandFromDrone = Serial.readStringUntil('\n');
    //boolean enviado = false;
    if (iAmConnected) {
      mesh.sendBroadcast(commandFromDrone);
      //enviado = true;
      Serial.print("Enviado: ");
      Serial.println(commandFromDrone);
    }
    //if(!enviado){
    //  Serial.print("Não foi enviado enviado: ");
    //  Serial.println(commandFromDrone);
    //}
    //DynamicJsonDocument ackDrone(1024);
    //deserializeJson(ackDrone, commandFromDrone);
    //serializeJson(ackDrone, Serial); 
  }
